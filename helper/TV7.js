      ...
      HelperTV7.fetchGlobalEmotes(items).finally(() => {
        ...
        for (let userID in items.tv7Emotes) {
          if (items.tv7Emotes.hasOwnProperty(userID)) {
            ...
            splitdev.innerHTML = `<div class="stickers__info" style="top:-10px"><div class="stickers__info__line"></div><div class="stickers__info__text"> ${typeof tv7Users[userID].username == 'undefined' ? userID : tv7Users[userID].username} </div><div class="stickers__info__line"></div></div><div class="stickers__line"></div>`
  ...
  replaceText(text) {
    ...
    for (let word of split) {
      ...
      if (HelperTV7.emotes[word]) {
        ...
        word = `<div class="bttv-emote tooltip-wrapper" ${settings.wasd.hoverTooltipEmote ? `title=" Смайл:&nbsp;${word} <br> ${typeof tv7Users[user].username == 'string' ? `Канал: ${tv7Users[user].username} <br> Эмоции на канале 7TV` : 'Общедоступный 7TV'} "` : `` }> <img class="stickerovg tv7 small" style="vertical-align: middle; width: auto!important;" src="https://cdn.7tv.app/emote/${this.emotes[word]}/${size}x" alt="${word}" /> <span class="chat-message-text stickertext stickerovg_text">Стикер</span> </div>`;
      }
