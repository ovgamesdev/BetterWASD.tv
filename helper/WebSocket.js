const socket = {
  ...
  addWebSocket_history(JSData) {
    ...
    isSub = (JSData) => {
      if (JSData) {
        let role = false;
        if (JSData.other_roles)
          for (let rol of JSData.other_roles) {
            if (rol == "CHANNEL_SUBSCRIBER") role = true;
          }
    ...
    isOwner = (JSData) => {
      if (JSData) {
        ...
        if (JSData.other_roles)
          for (let rol of JSData.other_roles) {
            if (rol == "CHANNEL_OWNER") role = true;
          }
    ...
    isPromoCodeWin = (JSData) => {
      if (JSData) {
        ...
        if (JSData.other_roles)
          for (let rol of JSData.other_roles) {
            if (rol == "PROMO_CODE_WINNER") role = true;
          }
