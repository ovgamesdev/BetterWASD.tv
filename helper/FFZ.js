const HelperFFZ = {
    ...
    replaceText(text) {
        ...
        for (let word of split) {
            ...
            if (this.emotes[word]) word = `<img class="stickerovg ffz small" style="vertical-align: middle; width: auto!important;" src="https://cdn.frankerfacez.com/emote/${this.emotes[word]}/${size}" alt="${word}" title="${word}" /> <span class="chat-message-text stickertext stickerovg_text">Стикер</span>`;
    ...
    restoreSettings(items) {
        return new Promise((resolve, reject) => {
            ...
            chrome.storage.local.set({ ffzUsers:items.ffzUsers, ffzEmotes:{} });

            let l = 0
            let i = 0

            for(let userID in items.ffzUsers) { l++ }
