const HelperFFZ = {
    ...
    for (let word of split) {
      ...
      if (HelperFFZ.emotes[word]) {
        ...
        let title = ` Смайл:&nbsp;${word} <br> ${
          typeof ffzUsers[user]?.username == "string"
            ? `Канал:&nbsp;${ffzUsers[user]?.username} <br> Эмоции на канале FFZ`
            : "Общедоступный FFZ"
        } `;
  ...
  addToChatMenu() {
    document
      ...
      .insertAdjacentHTML("beforeend", `<div class="option ffz-emoji"><i class="ovg-icon-ffz" style="pointer-events: none;"></i></div>`);
